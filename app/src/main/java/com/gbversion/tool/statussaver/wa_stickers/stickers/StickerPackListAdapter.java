...
public class StickerPackListAdapter extends RecyclerView.Adapter<StickerPackListItemViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final StickerPackListItemViewHolder viewHolder, final int index) {
        ...
        for (int i = 0; i < actualNumberOfStickersToShow; i++) {
            final ImageView rowImage = (ImageView) LayoutInflater.from(context).inflate(R.layout.sticker_packs_list_image_item, viewHolder.imageRowView, false);
            Glide.with(context)
                    .load(StickerPackLoader.getStickerAssetUri(pack.identifier, pack.getStickers().get(i).imageFileName))
                    .into(rowImage);
